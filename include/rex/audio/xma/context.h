...
class XmaContext {
 public:
  ...
  // [XMA fix] All of these are now virtual so XmaContextLegacy and
  // XmaContextNew can override them with their own decoding logic.
  // The base class provides empty default implementations so only
  // the subclass that actually uses each method needs to override it.
 ...
 protected:
  // [XMA fix] ConvertFrame is shared by both decoder variants (legacy and new)
  // so it stays here in the base class rather than being duplicated.
  static void ConvertFrame(const uint8_t** samples, bool is_two_channel, uint8_t* output_buffer);

  // [XMA fix] All members that both decoder subclasses need are now protected
  // instead of private so they can access them directly without extra accessors.
