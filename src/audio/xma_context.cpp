...
// [XMA fix] XmaContext is now a base class. The constructor and destructor
// only handle shared state — ffmpeg cleanup moved to the subclasses that
// actually allocate the ffmpeg handles (XmaContextLegacy, XmaContextNew).
...
bool XmaContext::Block(bool poll) {
  // [XMA fix] Block() tries to acquire the context mutex to confirm no decode
  // is currently running. poll=true returns immediately if the lock is busy,
  // poll=false waits until it's free. Used by the Lock register handler to
  // ensure the game doesn't touch context data mid-decode.
...
void XmaContext::ConvertFrame(const uint8_t** samples, bool is_two_channel, uint8_t* output_buffer) {
  // [XMA fix] ConvertFrame lives in the base class because both XmaContextLegacy
  // and XmaContextNew use the exact same SSE2-optimized float-to-int16 conversion.
  // Keeping it here avoids duplicating ~60 lines of SIMD code in both subclasses.
